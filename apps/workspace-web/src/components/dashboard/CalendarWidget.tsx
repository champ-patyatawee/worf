  ...
  return (
    <div className="min-h-full p-4 flex flex-col" style={{ backgroundColor: '#FFFBEB' }}>
      ...
      <div className="grid grid-cols-7 gap-0.5 mb-1">
        {DAYS.map(d => (
          <div key={d} className="text-center text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase py-0.5">{d}</div>
        ))}
      </div>
            ...
            <div key={i} className="flex items-center justify-center">
              {day !== null && (
                <span className={`w-6 h-6 flex items-center justify-center rounded-full text-[11px] font-bold ${
                  isToday
                    ? 'bg-[var(--color-accent-primary)] text-white border-2 border-[#0D0D0D] shadow-[2px_2px_0px_#0D0D0D]'
