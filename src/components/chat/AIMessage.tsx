...
export function AIMessage({ message, isStreaming }: { message: ChatMessage; isStreaming?: boolean }) {
  ...
  return (
    <div className={`flex gap-3 px-4 py-3 ${isUser ? "" : "bg-[var(--color-bg-tertiary)]"}`}>
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="text-xs font-bold mb-1" style={{ color: isUser ? "var(--color-accent-primary)" : "var(--color-text-secondary)" }}>
          {isUser ? "You" : "AI"}
          {isStreaming && <span className="ml-2 animate-pulse" style={{ color: "var(--color-accent-primary)" }}>...</span>}
        </div>
